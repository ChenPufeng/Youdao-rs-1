...
fn parse_arguments(args: Args) -> Option<App> {
    ...
    while let Some(arg) = args.pop() {
        if arg == "-v" { is_voice = true; continue; }
        if arg == "-m" { is_more = true; continue; }
        words.push(arg)
    }
...
fn display_usage() {
    ...
    println!("{}", Colour::Blue.paint("YoudaoCLI <word(s) to query>        Query the word(s)"));
    println!("{}", Colour::Blue.paint("YoudaoCLI <word(s) to query> -v     Query with speech"));
    println!("{}", Colour::Blue.paint("YoudaoCLI <word(s) to query> -v     Query with speech"));
}
